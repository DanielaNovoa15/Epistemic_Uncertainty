...
fcNom = 24500                                          # fc del concreto 
xlocNorm = [3.02/12.62,6.22/12.62,9.42/12.62,12.62/12.62] # altura normalizada
...
# filename = str(modelname)+'_results'+str(Nmod)+'Mod'  # Nombre del archivo a cargar
...
# indnf = (lim_col >= dfd_1['dmax_aj']) & (dfd_1['t_record'] >= dfd_1['tgm'])      # indices de los registros que no fallaron                     
...
# dfnf = dfd_1.query('t_record >= tgm & @lim_col >= dmax_aj')
# print('---------------------------------------')
...
# #     filename = str(modelname)+'_resultscomp'+str(Nmod)+'Mod'+'_FE'+str(FE)+'_parte'+str(indx)
# #     outfile = open(filename,'wb')
...
# # filename = str(modelname)+'_resultsComp'+str(Nmod)+'Mod' # Nombre del archivo a almacenar
# # outfile = open(filename,'wb')
# # pickle.dump(resultcomp,outfile)
# # outfile.close()
#%% CARGAR RESULATDOS DEL ANÁLISIS
filename = str(modelname)+'_resultsComp'+str(Nmod)+'Mod'  # Nombre del archivo a cargar
infile = open(filename,'rb')
...
colors = ['#0343DF','#008080','#A52A2A','#CE7B42','#0343DF','#008080','#A52A2A','#CE7B42']
lineStyle = ['solid','solid','solid','solid','dashdot','dashdot','dashdot','dashdot']
nameleg = ['1st Floor (300 Models)','2nd Floor (300 Models)','3rd Floor (300 Models)','4th Floor (300 Models)']
nameleg2 = ['1st Floor (Nominal Model)','2nd Floor (Nominal Model)','3rd Floor (Nominal Model)','4th Floor (Nominal Model)']
for i in range(Npisos):
    plt.plot(TcVar,MedianPGA[:,i],label=nameleg[i],color=colors[i],linestyle=lineStyle[i],linewidth=1.0)
    plt.plot(TcVar,MedianNomPGA[:,i],label=nameleg2[i],color=colors[i+4],linestyle=lineStyle[i+3],linewidth=1.0)
plt.xlabel('Period [s]')
plt.ylabel('Acceleration/PGA')
plt.title('a. Floor amplification factor - Normalized by the PGA',fontweight='bold')
plt.legend(title='Legend',loc='lower center',ncol=2,handletextpad=0.0,handleheight=0.5,bbox_to_anchor=(0.5, -0.54))
...
for i in range(Npisos):
    ...
    plt.plot(TcVar,MedianNom[:,i],label=nameleg2[i],color=colors[i+4],linestyle=lineStyle[i+4],linewidth=1.0)
plt.xlabel('Period [s]', fontsize = 12.5)
plt.ylabel('PCA/PFA', fontsize = 12.5)
plt.title('',fontweight='bold', fontsize = 14)
plt.legend(title='Legend',loc='lower center',ncol=2,handletextpad=0.0,handleheight=0.5,bbox_to_anchor=(0.5, -0.55))
plt.xticks(np.arange(0.0, 3.3, step=0.3))
plt.show()
# =============================================================================
...
for i in range(Npisos):
    ...
    plt.plot(TcVar,MedianNomComp[:,i],label=nameleg2[i],color=colors[i+4],linestyle=lineStyle[i+3],linewidth=1.0)
ax.set_xlabel('Period [s]')
ax.set_ylabel('PCA [m/s$^{2}$]')
ax.set_title('c. Component acceleration',fontweight='bold')
legend = ax.legend(title='Legend',loc='lower center',ncol=2,handletextpad=0.0,handleheight=0.5,bbox_to_anchor=(0.5, -0.54))
...
for i in range(Npisos):
    ...
    plt.plot(TcVar,MedianNomCompG[:,i],label=nameleg2[i],color=colors[i+4],linestyle=lineStyle[i+4],linewidth=1.0)
ax.set_xlabel('Period [s]', fontsize = 12.5)
ax.set_ylabel('PCA [g]', fontsize = 12.5)
ax.set_title('',fontweight='bold', fontsize = 14)
legend = ax.legend(title='Legend',loc='lower center',ncol=2,handletextpad=0.0,handleheight=0.5,bbox_to_anchor=(0.5, -0.55))
# legend.get_frame().set_facecolor('white')
# for text in legend.get_texts():
#     text.set_fontsize(8)
ax.set_xticks(np.arange(0.0, 3.3, step=0.3))
...
colors = ['orange','green','#A52A2A','#00B0F0']
nameleg = ['1st Floor','2nd Floor ','3rd Floor','4th Floor']
fig, ax = plt.subplots()
for i in range(Npisos):
...
# for i in range(Nper):
#     for j in range(Npisos):
#         ylist[i,j] = xlocNorm[j] #Altura normalizada
#         xlist[:,j] = TcVar/TOpSees

# plt.rcParams["figure.figsize"] = (8, 6)
# fig = plt.figure()
# axes = fig.add_subplot(projection='3d')
# axes.plot_surface(xlist, ylist, Median,cmap='plasma')
# axes.set_title('b. MORUMBI',fontweight='bold')
