...
#     t = Nsteps[i]*DTs[i]                                # Calcula el tiempo del sismo
...
# converg = t_record > 0.98*tgm                          # 1 si converge, 0 si no
...
# dic = {'modelo':Model, 'record':record, 't_record':t_record,'tgm':tgm, 'max_d': max_d, 'fc': fc, 'fy': fy, 'converg':converg, 'dmax_aj':dmax_aj}  
# dfd = pd.DataFrame(dic)
# print('---------------------------------------')
# print('-------DataFrame de Datos creado-------')
# print('---------------------------------------')
#%% Guardar resultados
# filename = str(modelname)+'dataframederivas'
# outfile = open(filename,'wb')
# pickle.dump(dfd,outfile)
# outfile.close()
#%% CARGAR DATAFRAME DE DERVIAS
filename = str(modelname)+'dataframederivas'   # Nombre del archivo a cargar
infile = open(filename,'rb')
...
g.despine(left=True, bottom=True)
#%% VALORES DE FC Y FY
fclist = prob_lig[namefc].tolist()
fylist = prob_lig[namefy].tolist()
#%% Guardar resultados
# #----------- resultados fclist -----------
# filename = str(modelname)+'fclist_def'
# outfile = open(filename,'wb')
# pickle.dump(fclist,outfile)
#----------- resultados fylist -----------
filename = str(modelname)+'fylist_def1'
outfile = open(filename,'wb')
