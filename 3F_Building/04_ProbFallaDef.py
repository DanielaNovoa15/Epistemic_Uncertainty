...
# timegm = []                                            # Almacena el tiempo del sismo
# for i in range(len(Nsteps)):
#     t = Nsteps[i]*DTs[i]*0.98                               # Calcula el tiempo del sismo
#     timegm.append(t)
# tgm = timegm*Nmod                                      # Almacena en una lista el tiempo del sismo para la cantidad de modelos analizados

# Mod,record,max_d,fc,fy,acel,Dts,acel_rec,t_record = [],[],[],[],[],[],[],[],[] # modelo, registro, maxima deriva
...
#     Mod.append(modelo)                                 # Almacena el modelo de análisis
#     record.append(res[9]+1)                            # Almacena el registro del análisis
#     t_record.append(np.around(res[0][-1],2))           # Almacena el tiempo al que llegó el registro 
...
#     acel_rec.append(np.hstack((0,np.loadtxt(records[res[9]])*9.81))) # Almacena la acelración del registro
...
# dic = {'modelo':Mod, 'record':record, 't_record':t_record,'tgm':tgm, 'max_d': max_d, 'fc': fc, 'fy': fy, 'converg':converg, 'dmax_aj':dmax_aj}
# dfd = pd.DataFrame(dic)
# print('---------------------------------------')
# print('-------DataFrame de Datos creado-------')
# print('---------------------------------------')
#%% GUARDAR RESULTADOS DEL ANÁLISIS
# filename = str(modelname)+'dataframederivasV2' # Nombre del archivo a almacenar
# outfile = open(filename,'wb')
# pickle.dump(dfd,outfile)
# outfile.close()
# CARGAR RESULTADOS
filename = str(modelname)+'dataframederivasV2'
infile = open(filename,'rb')
...
cmap = sb.cubehelix_palette(n_colors=6,start=6,rot=-1.0,as_cmap=True)
g = sb.relplot(
    data = prob_lig,
    x = 'Model', y = 'P(SDR$_{max}$>DS$_{limit}$)',
    hue = namefc, size = namefy,
    palette = cmap, sizes = (5,130),).set(title='a. ISAI (3 stories)')
