...
node_record = [1000,1001,1002,1003,1004,1005,1006,1007,1008,1009,1010,1011,1012,1013,1014] # Nodos guardar información
ele_record = [10000]                                          # Elementos guardar información
node_control = 4014 
# ---------------------Parámetros del modelo dinámico---------------------
FE = 1.0                                                   # Factor escalar del modelo
n = 299                                                    # Número de modelos
...
aa = []
for i in range(len(SpectrumFactor)):
    aa.append(SpectrumFactor[i]*9.81)
    
np.savetxt('SpectrumFactorANTAKYA.txt', aa, delimiter=',')
#%% VARIABILIDAD DEL MATERIAL
index2 = range(len(records))                               # Rango terremotos
mfc = 28000                                                # Valor medio de datos fc
cv = 0.15
...
filename = 'Variables_'+str(modelname)                       # Nombre del archivo a cargar
infile = open(filename,'rb')
variables = pickle.load(infile)
infile.close()
#%% SEGMENTAR LISTA VARIABLES
modseg = 1                # Segmentar en 3. 
varseg = variables[(modseg-1)*100:100*modseg]
...
stime = time.time()
resultados = Parallel(n_jobs=num_cores)(delayed(runDyn)(ii,ff) for ii in index2 for ff in varseg) # loop paralelo
etime = time.time()
ttotal = etime - stime
print('---------------------------------------')
...
print('---------------------------------------') 
#%% Guardar resultados
# filename = 'Results'+str(n)+'(seg'+str(modseg)+')_Mod'+'_FE'+str(FE)
